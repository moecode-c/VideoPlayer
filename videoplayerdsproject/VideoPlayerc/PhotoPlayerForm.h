...
namespace VideoPlayerc {
    ...
    private:
        ...
        void InitializeComponent(void)
        {
            ...
            try {
                // load background relative to the application startup folder so the project is portable
                String^ bgPath = System::IO::Path::Combine(Application::StartupPath, "Images\\photoBackground.png");
                if (System::IO::File::Exists(bgPath))
                {
                    this->pictureBoxBackground->Image = Image::FromFile(bgPath);
                }
                else
                {
                    // fallback to a relative path in case running from IDE where working dir differs
                    String^ alt = System::IO::Path::Combine(System::IO::Directory::GetCurrentDirectory(), "Images\\photoBackground.png");
                    if (System::IO::File::Exists(alt))
                        this->pictureBoxBackground->Image = Image::FromFile(alt);
                }
            }
            catch (...) {
            ...
            try {
                ...
                // Use the user's explicit play button image path if present (relative)
                {
                    String^ playPath = System::IO::Path::Combine(base, "play btn.png");
            ...
            // lblPhotoName
            // give the title a navy blue background to make it stand out
            this->lblPhotoName->BackColor = System::Drawing::Color::FromArgb(0, 0, 128); // navy
            ...
            // lblPhotoInfo
            // set a dark blue background so the "Load photos to get started" area is a dark blue rectangle
            this->lblPhotoInfo->BackColor = System::Drawing::Color::FromArgb(24, 55, 114);
            this->lblPhotoInfo->Font = (gcnew System::Drawing::Font(L"Segoe UI", 10));
            this->lblPhotoInfo->ForeColor = System::Drawing::Color::White;
            ...
            // Ensure background is behind all other controls so buttons are visible
