...
VideoList::Node::Node(String^ path, String^ name)
{
    ...
    videoName = name;
    next = nullptr;
    addedTime = DateTime::Now;  // Store when video was added
}
...
// ------------------------------
// SORT ALPHABETICALLY (NEW - FOR BUTTON)
// ------------------------------
...
// ------------------------------
// SORT BY TIME ADDED (NEW - FOR BUTTON)
// ------------------------------
void VideoList::sortByTime(ListBox^ box)
{
    if (isEmpty() || size <= 1) return;
    ...
    do
    {
        ...
        while (next != nullptr)
        {
            if (DateTime::Compare(curr->addedTime, next->addedTime) > 0)
            {
                swapped = true;
...
void VideoList::clearAll(ListBox^ box)
{
    ...
    System::Windows::Forms::DialogResult result = MessageBox::Show(
        "Are you sure you want to remove all videos from the playlist?",
        "Clear All Videos",
        MessageBoxButtons::YesNo,
    ...
    MessageBox::Show("All videos have been removed from the playlist.",
        "Cleared",
...
// ------------------------------
// ARRANGE ALPHABETICALLY (LEGACY - called on add)
// ------------------------------
void VideoList::arrangeAlphabetically(ListBox^ box)
{
    sortAlphabetically(box);
}
